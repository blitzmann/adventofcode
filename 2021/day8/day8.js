...
input = text.split("\n").map(x=>x.trim()).map(c=>c.split(" | ").map(x=>x.split(" ")));

part1 = 0;
for (let [_, output] of input){
    part1 += output.map(x=>x.split("").length).filter(x=>x==7 || x == 4 || x == 2 || x == 3).length
...
for (let [digits, output] of input){
    ...
    for (let test of digits.filter(x=>x.size === 5)) {
        if (test.has(a) && test.has(c) && test.has(d) && test.has(f) && !test.has(b)) {
            // found '3'
    ...
    for (let test of digits.filter(x=>x.size === 5)) {
        // found '2'
        if (test.has(a) && test.has(c) && test.has(d) && !test.has(f) && test.has(g)) {
    ...
    let outputInt = parseInt(output.map(x=>x.split("").map(y=>mapping.get(y)).sort()).map(x=>primaryMapping.get(x.join("")).toString()).join(""))
    part2 += outputInt
}
console.log(part2)
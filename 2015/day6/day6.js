...
const n = 1000;
let grid = Array.from(Array(n), () => new Array(n).fill(false));

const instruction = (cmd, x1, x2, y1, y2) => {
    for (let x = x1; x <= x2; x++) {
        for (let y = y1; y <= y2; y++) {
            if (cmd == 'toggle') {
                grid[y][x] = !grid[y][x];
...
for (let line of input.split('\n')) {
    let [_, cmd, x1, y1, x2, y2] = line.match(
        /(\w+) (\d+),(\d+) through (\d+),(\d+)/
    );
    instruction(cmd, Number(x1), Number(x2), Number(y1), Number(y2));
}

console.log(
    grid.map((x) => x.filter(Boolean).length).reduce((a, b) => a + b, 0)
);

// Part 2
grid = Array.from(Array(n), () => new Array(n).fill(0));

const instruction2 = (cmd, x1, x2, y1, y2) => {
    for (let x = x1; x <= x2; x++) {
        for (let y = y1; y <= y2; y++) {
            if (cmd == 'toggle') {
                grid[y][x] = grid[y][x] + 2;
...
for (let line of input.split('\n')) {
    ...
    instruction2(cmd, Number(x1), Number(x2), Number(y1), Number(y2));
}

console.log(
    grid.map((x) => x.reduce((a, b) => a + b, 0)).reduce((a, b) => a + b, 0)
);
